const totalBudgetInput = document.getElementById('totalBudgetInput');
let totalBudget = 0;

let rowCounter = 1;


function addRowBudget(event){
    
    event.preventDefault();
    const td = document.createElement('tr');
    td.id = `fila-${rowCounter}`
    td.innerHTML = `<td class=""><input type="text" class="form-control " placeholder="Material..." id="rowBudget-${rowCounter}"></td>
                    <td class="ps-2" style="width:20%;"><input type="number" class="form-control amountBudget" placeholder="Importe"></td>
                    <td class="text-center"> 
                    <button type="button" class="removeRowBtn btn btn-danger" onclick="(removeRowBudget(${rowCounter}))">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6" style="color: white; height:27px">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                            </svg>
                        </button>   
                    </td>`
    const addBtn = document.getElementById('parentAddBtn');

    addBtn.parentNode.insertBefore(td, addBtn)
    rowCounter++;
    const newAmountsInputs = document.querySelectorAll('.amountBudget');
    newAmountsInputs.forEach(amount => {
        amount.addEventListener('change', handleChangeAmounts);
    });
}
function addRowBudgetService(){
    const td = document.createElement('tr');
    td.id = `fila-${rowCounter}`
    td.innerHTML = `<td class="">
                        <input type="text" class="form-control " placeholder="Material..." id="rowBudget-${rowCounter}">
                        <input type="number" id="rowBudget-${rowCounter}-Id" hidden></td>
                    <td class="ps-2" style="width:20%;"><input type="number" class="form-control amountBudget" placeholder="Importe"></td>
                    <td class="text-center"> 
                    <button type="button" class="removeRowBtn btn btn-danger" onclick="(removeRowBudget(${rowCounter}))">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6" style="color: white; height:27px">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                            </svg>
                        </button>   
                    </td>`
    const addBtn = document.getElementById('parentAddBtn');

    addBtn.parentNode.insertBefore(td, addBtn)
    rowCounter++;
    const newAmountsInputs = document.querySelectorAll('.amountBudget');
    newAmountsInputs.forEach(amount => {
        amount.addEventListener('change', handleChangeAmounts);
    });
}

function removeRowBudget(id){
    const idRow = `fila-${id}`
    const row = document.getElementById(idRow);
    if(row){
        parentRow = row.parentNode;
        parentRow.removeChild(row)
    }
    handleChangeAmounts();
}

//This will tally up the total of amounts.
const amountsInputs = document.querySelectorAll('.amountBudget');

amountsInputs.forEach(amount => {
    amount.addEventListener('change', handleChangeAmounts)
})

function handleChangeAmounts(event){
    totalBudget = 0;
    const amountsInputs = document.querySelectorAll('.amountBudget');
    amountsInputs.forEach(amount => {
        let valor = parseFloat(amount.value);
        if(!isNaN(valor)){
            totalBudget += valor;
        }
    })
    totalBudgetInput.value = totalBudget.toFixed(2);
}