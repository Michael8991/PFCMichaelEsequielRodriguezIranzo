...
function getRecords($conn, $items_per_page, $offset, $company_id, $searchTerm = '') {
    try {
        ...
        //     WHERE Budgets.CompanyID = :companyID
        // ';
...
// function getRecords($conn, $items_per_page, $offset, $company_id) {
//     try {
//         $stmt = $conn->prepare('
//             SELECT * FROM Budgets
//             JOIN Projects ON Budgets.ProjectID = Projects.ProjectID 
...
//         ');
//         $stmt->bindValue(':companyID', $company_id, PDO::PARAM_INT);
//         $stmt->bindValue(':limit', $items_per_page, PDO::PARAM_INT);
//         $stmt->bindValue(':offset', $offset, PDO::PARAM_INT);
//         $stmt->execute();
//         $records = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
// $company_id = getCompanyID($conn, $user); // Retrieve the CompanyID first
// $total_items = countAllRecords($conn, $company_id); // Then use the CompanyID to count records
...
$total_pages = getTotalPages($total_items, $items_per_page);
$offset = getOffset($page, $items_per_page);
// Aquí deberías manejar la respuesta JSON según sea necesario
