...
function countAllRecords($conn, $company_id) {
    try {
        $stmt = $conn->prepare('SELECT COUNT(*) FROM Services WHERE CompanyID = :company_id');
...
function getRecords($conn, $items_per_page, $offset, $company_id, $searchTerm = '') {
    try {
        ...
        if (empty($searchTerm)) {
            $sql = 'SELECT * FROM Services WHERE CompanyID = :companyID LIMIT :limit OFFSET :offset';
            $stmt = $conn->prepare($sql);
            ...
            $sql = 'SELECT * FROM Services WHERE CompanyID = :companyID AND (ServiceName LIKE :searchTermWildcard) LIMIT :limit OFFSET :offset';
            $stmt = $conn->prepare($sql);
            $stmt->bindParam(':companyID', $company_id, PDO::PARAM_INT);
