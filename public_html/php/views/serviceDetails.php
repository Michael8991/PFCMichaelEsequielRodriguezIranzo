<?php 
    ...
    try {
        $sqlCompany = "SELECT ServiceName, ServiceDescription, UnitPrice, UnitFormat, CompanyID FROM Services WHERE ServiceID = :serviceID";
        $stmt = $conn->prepare($sqlCompany);
    ...
    <div class="home">
        ...
        <div class="home-container">    
            <div class="form-container">
                <form class="addBudgetForm" id="updateServiceForm" action="../ServicesOPs/updateService.php" method="POST">
                <input type="hidden" name="ServiceID" value="<?php echo $serviceID; ?>">
                    <div class="section-form" id="generalDatasSection">
                        <div class="mb-3">
                            <label for="first_name" class="form-label">Nombre:</label>
                            <input type="text" class="form-control" id="service_name" name="service_name" value="<?php echo $ServiceName ?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="last_name" class="form-label">Descripción:</label>
                            <input type="text" class="form-control" id="service_description" value="<?php echo $ServiceDescription ?>" name="service_description">
                        </div>
                        <div class="mb-3">
                            <label for="email" class="form-label">Precio unidad:</label>
                            <input type="number" class="form-control" id="unit_price" value="<?php echo $UnitPrice ?>" name="unit_price">
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="unit_format" aria-label="Default select example"  required>
                                <?php 
                                echo '<option value="" disabled' . (empty($UnitFormat) ? ' selected' : '') . '>Seleccione un formato</option>';
                                echo '<option value="Hora"' . ($UnitFormat === 'Hora' ? ' selected' : '') . '>Hora</option>';
                                echo '<option value="Jornal"' . ($UnitFormat === 'Jornal' ? ' selected' : '') . '>Jornal</option>';
                                ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit" class="btn btn-primary btn-success" form="updateServiceForm">Aceptar</button>
                            </div>
                        </div>
                    </div>
                </div>
