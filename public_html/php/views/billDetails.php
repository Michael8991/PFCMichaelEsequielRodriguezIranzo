<?php
    ...
    try {
        // Consulta SQL
        $consultaCompleta = "SELECT * FROM bills 
        ...
        $consultaItems = "SELECT * FROM BillItems WHERE BillID = :bill_id";
    ...
    foreach($resultados as $resultado){
        $BillDateIssued = $resultado['BillDateIssued'];
        $CustomerID = $resultado['CustomerID'];
        $BillDueDate = $resultado['BillDueDate'];
        $TotalBillAmount = $resultado['TotalBillAmount'];
        $BillStatus = $resultado['BillStatus'];
        $BillPaymentMethod = $resultado['BillPaymentMethod'];
        $BillNotes = $resultado['BillNotes'];
        $BillCreator = $resultado['BillCreator'];
        $BillCreatedAt = $resultado['BillCreatedAt'];
        $CompanyID = $resultado['CompanyID'];
        $ProjectID = $resultado['ProjectID'];
        $CustomerName = $resultado['CustomerName'];
        $BillCreatorName = $resultado['BillCreatorName'];
        $BillCreatorName = $resultado['BillCreatorName'];
        $ProjectName = $resultado['ProjectName'];
        $billNumber = $resultado['billNumber'];
    }
    if($CompanyID != $company_user){
    ...
    <div class="home">
        ...
        <div class="home-container">    
            <div class="form-container">
                <form class="addBillForm" id="addBillForm" action="../BillsOPs/updateBill.php" method="POST">
                <input type="hidden" name="bill_id" value="<?php echo $bill_id; ?>">
                    <div class="section-form" id="generalDatasSection">
                        <h5>Datos generales</h5>
                        <div class="d-flex w-100">
                            <?php $nombreUsuario = $_SESSION['user']; echo "<p class='align-item-start w-50' style='color:grey; font-size:15px;'> Creado por: <i>" .$BillCreatorName. " </i> </p>"  ?>
                            <p class="w-50 text-end" style="color:grey; font-size:15px;">Número de factura: <?php echo $billNumber ?></p>
                        </div>
                        <div class="rowBudget">
                            <label for="fechaEmision">Fecha de emisión:</label>
                            <input type="date" class="form-control" id="fechaEmision" name="fechaEmisionInput" value="<?php echo $BillDateIssued ?>" required>
    
                            <label for="fechaValidez">Fecha de validez:</label>
                            <input type="date" class="form-control" id="fechaValidez" name="fechaValidezInput" value="<?php echo $BillDueDate ?>" required>
                        </div>
                        <div class="rowBudget">
                            <label for="searchProjectName">Proyecto:</label>
                            <input autocomplete="off" class="form-control search-box" type="text" id="projectInputName" placeholder="Seleccione un proyecto" value="<?php echo $ProjectName ?>"  Disabled readonly>
                            <input autocomplete="off" type="text" name="projectInputID" id="projectInputID" value="<?php echo $ProjectID ?>" hidden>
                            <label for="searchCustomerInput">Cliente:</label>
                            <input autocomplete="off" type="text" name="customerInputID" id="customerInputID" value="<?php echo $CustomerID ?>" hidden>
                            <input autocomplete="off" class="form-control search-box" type="text" name="searchCustomerInput" id="searchCustomerInput" value="<?php echo $CustomerName ?>" Disabled readonly>
                            ...
                            <tr>
                                <td colspan="3">
                                    <div class="input-group">
                                        ...
                                        <input name="totalAmountBudget" type="number" class="form-control" readonly placeholder="0,00" value="<?php echo $TotalBillAmount ?>" id="totalBudgetInput">
                    ...
                    <div class="section-form">
                        ...
                        <div class="rowBudget">
                            <div class="form-floating" style="width: 100% !important">
                                <textarea name="floatingTextarea2" class="form-control col-12" placeholder="Introduce los materiales no incluidos" id="floatingTextarea2" style="height: 100px;"><?php echo $BillNotes ?></textarea>
                        ...
                        <div class="input-group mb-3">
                            <select class="form-select" name="paymentMethod" aria-label="Default select example" required>
                                <?php
                                echo '<option value="" disabled' . (empty($BillPaymentMethod) ? ' selected' : '') . '>Seleccione un método</option>';
                                echo '<option value="Tarjeta"' . ($BillPaymentMethod === 'Tarjeta' ? ' selected' : '') . '>Tarjeta</option>';
                                echo '<option value="Efectivo"' . ($BillPaymentMethod === 'Efectivo' ? ' selected' : '') . '>Efectivo</option>';
                                echo '<option value="Cheque"' . ($BillPaymentMethod === 'Cheque' ? ' selected' : '') . '>Cheque</option>';
                                echo '<option value="Transferencia bancaria"' . ($BillPaymentMethod === 'Transferencia bancaria' ? ' selected' : '') . '>Transferencia bancaria</option>';
                                ?>
                            </select>

                            <select class="form-select ms-1" name="paymentStatus" aria-label="Default select example" required>
                                <?php 
                                echo '<option value="" disabled' . (empty($BillStatus) ? ' selected' : '') . '>Seleccione el estado de la factura</option>';
                                echo '<option value="Pagada"' . ($BillStatus === 'Pagada' ? ' selected' : '') . '>Pagada</option>';
                                echo '<option value="Pendiente"' . ($BillStatus === 'Pendiente' ? ' selected' : '') . '>Pendiente</option>';
                                ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit" class="btn btn-primary btn-success" form="addBillForm">Aceptar</button>
                            </div>
                        </div>
                    </div>
                </div>
