...
function countAllRecords($conn, $company_id) {
    try {
        $stmt = $conn->prepare('SELECT COUNT(*) FROM Bills WHERE CompanyID = :company_id');
...
function getRecords($conn, $items_per_page, $offset, $company_id, $searchTerm = '') {
    try {
        ...
        if (empty($searchTerm)) {
            $sql = 'SELECT * FROM Bills WHERE CompanyID = :companyID LIMIT :limit OFFSET :offset';
            $stmt = $conn->prepare($sql);
            ...
            $sql = 'SELECT * FROM Bills WHERE CompanyID = :companyID AND (ProjectName LIKE :searchTermWildcard OR CustomerName LIKE :searchTermWildcard OR billNumber LIKE :searchTermWildcard) LIMIT :limit OFFSET :offset';
            $stmt = $conn->prepare($sql);
            $stmt->bindParam(':companyID', $company_id, PDO::PARAM_INT);
